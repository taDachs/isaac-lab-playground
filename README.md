...
- Throughout the repository, the name `isaac-lab-playground` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of isaac-lab-playground (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e source/isaac-lab-playground
```
...
We provide an example UI extension that will load upon enabling your extension defined in `source/isaac-lab-playground/isaac-lab-playground/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/isaac-lab-playground"
    ]
}
```
